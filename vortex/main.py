...
async def _initialise_runtime() -> None:
    ...
    roadmap_planner = RoadmapPlanner(nlp_engine=ai_nlp, planner=planner)
    pipeline_manager = PipelineManager(monitor=perf_monitor)
    project_manager = ProjectManager(pipeline_manager, roadmap_planner, monitor=perf_monitor)
    team_manager = TeamManager(team_analytics=team_analytics)
    web_ui = WebUI()
