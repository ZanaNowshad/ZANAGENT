...
class TUIActionCenter:
    ...
    async def _cmd_project(self, command: SlashCommand) -> CommandResult:
        manager = getattr(self._runtime, "project_manager", None)
        if manager is None:
            return CommandResult(message="Project manager unavailable")
        if not command.args:
            return CommandResult(message="Usage: /project <status|plan|release> [args]")
        action = command.args[0]
        project_id = command.args[1] if len(command.args) > 1 else self._git_root.name
        if action == "status":
            state = await manager.status(project_id)
            table = Table(title=f"Project {project_id}", expand=True)
            table.add_column("Milestone")
            table.add_column("Status")
            table.add_column("Due")
            for milestone in state.milestones[:6]:
                table.add_row(milestone.name, milestone.status, milestone.due or "-")
            metadata = {
                "project_status": {
                    ...
                    "summary": state.metadata.get("vision", ""),
                },
                "ledger": state.ledger_totals,
            }
            await self._record_event("project-status", {"summary": project_id}, metrics={"success": True})
            return CommandResult(
                message=f"Project {project_id} status loaded",
                renderable=table,
                metadata=metadata,
            )
        if action == "plan":
            sources = command.args[1:] or [str(self._git_root / "project.yml")]
            summary = await manager.plan_roadmap(project_id, sources)
            table = Table(title="Roadmap", expand=True)
            table.add_column("Milestone")
            table.add_column("Tasks")
            for milestone in summary.milestones:
                table.add_row(milestone.name, str(len(milestone.tasks)))
            metadata = {
                "project_status": {
                    "milestones": [
                        {
                            ...
                            "status": "planned",
                            "due": milestone.due,
                            "tasks": [task.__dict__ for task in milestone.tasks],
                        }
                        for milestone in summary.milestones
                    ],
                    "ledger": {},
                    "summary": f"Backlog size {summary.backlog_size}",
                }
            }
            await self._record_event("project-plan", {"summary": project_id}, metrics={"success": True})
            return CommandResult(
                message=f"Roadmap generated for {project_id}",
                renderable=table,
                metadata=metadata,
            )
        if action == "release":
            if len(command.args) < 2:
                return CommandResult(message="Usage: /project release <version>")
            version = command.args[1]
            summary_text = command.option("--summary") or "Release created via TUI"
            release = await manager.record_release(
                project_id,
                version,
                summary=summary_text,
                author=self._identity,
            )
            metadata = {"project_status": {"releases": [release.__dict__]}}
            await self._record_event("project-release", {"summary": version}, metrics={"success": True})
            return CommandResult(
                message=f"Release {version} captured",
                renderable=Text(f"Release {version} recorded", style="green"),
                metadata=metadata,
            )
        return CommandResult(message=f"Unknown project action: {action}")

    async def _cmd_pipeline(self, command: SlashCommand) -> CommandResult:
        manager = getattr(self._runtime, "pipeline_manager", None)
        if manager is None:
            return CommandResult(message="Pipeline manager unavailable")
        if not command.args:
            return CommandResult(message="Usage: /pipeline <run|status|logs|dashboard> <pipeline>")
        action = command.args[0]
        pipeline_name = command.args[1] if len(command.args) > 1 else "default"
        project_id = command.args[2] if len(command.args) > 2 else self._git_root.name
        if action == "run":
            stage = command.option("--stage") or command.option("-s")
            environment = command.option("--env") or command.option("-e")
            run = await manager.run(project_id, pipeline_name, stage, environment=environment)
            await self._record_event(
                "pipeline-run",
                {"summary": pipeline_name, "stage": stage or "all"},
                metrics={"success": run.summary == "success"},
            )
            metadata = {
                "pipeline_history": [stage.to_dict() for stage in run.stages],
                "summary": run.summary,
            }
            return CommandResult(
                message=f"Pipeline {pipeline_name} executed",
                renderable=run.rich_table(),
                metadata=metadata,
            )
        if action == "status":
            run = await manager.status(project_id, pipeline_name)
            if run is None:
                return CommandResult(message="No pipeline history available")
            metadata = {
                "pipeline_history": [stage.to_dict() for stage in run.stages],
                "summary": run.summary,
            }
        ...
        if action == "logs":
            logs = await manager.logs(project_id, pipeline_name)
            renderable = Syntax("\n".join(logs) or "No logs", "text")
            return CommandResult(message="Pipeline logs", renderable=renderable, plain_text=renderable.plain)
        if action == "dashboard":
            table = await manager.dashboard(project_id)
            metadata = {"pipeline_history": []}
            return CommandResult(message="Pipeline dashboard", renderable=table, metadata=metadata)
        return CommandResult(message=f"Unknown pipeline action: {action}")

    async def _cmd_release(self, command: SlashCommand) -> CommandResult:
        manager = getattr(self._runtime, "project_manager", None)
        if manager is None:
            return CommandResult(message="Project manager unavailable")
        if not command.args:
            return CommandResult(message="Usage: /release <tag|rollback> <version>")
        action = command.args[0]
        project_id = command.args[2] if len(command.args) > 2 else self._git_root.name
        version = command.args[1] if len(command.args) > 1 else ""
        if action == "tag":
            release = await manager.record_release(
                project_id,
                version,
                summary=command.option("--summary") or "Tagged via TUI",
                author=self._identity,
                status="ready",
            )
            metadata = {"project_status": {"releases": [release.__dict__]}}
            return CommandResult(
                message=f"Release {version} tagged",
                renderable=Text(f"Tagged {version}", style="green"),
                metadata=metadata,
            )
        if action == "rollback":
            release = await manager.rollback_release(
                project_id,
                version,
                reason=command.option("--reason") or "manual",
                actor=self._identity,
            )
            metadata = {"project_status": {"releases": [release.__dict__]}}
            return CommandResult(
                message=f"Release {version} rolled back",
                renderable=Text(f"Rolled back {version}", style="yellow"),
                metadata=metadata,
            )
        return CommandResult(message=f"Unknown release action: {action}")

    async def _cmd_govern(self, command: SlashCommand) -> CommandResult:
        manager = getattr(self._runtime, "project_manager", None)
        if manager is None:
            return CommandResult(message="Project manager unavailable")
        project_id = command.args[0] if command.args else self._git_root.name
        report = await manager.governance_audit(project_id)
        table = Table(title="Governance Audit", expand=True)
        table.add_column("Check")
        table.add_column("Status")
        for check in report.get("checks", []):
            table.add_row(check.get("name", "policy"), check.get("status", "pass"))
        await self._record_event("governance", {"summary": project_id}, metrics={"success": report.get("status") != "fail"})
        metadata = {"governance": report}
        return CommandResult(message="Governance audit complete", renderable=table, metadata=metadata)

    async def _cmd_broadcast(self, command: SlashCommand) -> CommandResult:
        manager = getattr(self._runtime, "team_manager", None)
        if manager is None:
            return CommandResult(message="Team manager unavailable")
        if not command.args:
            return CommandResult(message="Usage: /broadcast <message>")
        message = " ".join(command.args)
        await manager.broadcast(message, {"author": self._identity})
        await self._record_event("broadcast", {"summary": message}, metrics={"success": True})
        return CommandResult(message="Broadcast sent", renderable=Text(message, style="cyan"))
