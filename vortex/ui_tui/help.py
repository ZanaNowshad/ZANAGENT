...
PROJECT_COMMANDS = [
    ("/project init <path>", "Register repository for lifecycle management"),
    ("/project status", "Show milestones, ledger, and releases"),
    ("/project plan", "Generate roadmap from backlog"),
    ("/project release <version>", "Record release milestone"),
    ("/pipeline run <name>", "Execute build/test pipeline"),
    ("/pipeline dashboard", "Stream pipeline metrics"),
    ("/release tag <version>", "Create release record"),
    ("/release rollback <version>", "Rollback release"),
    ("/govern", "Run compliance audit"),
    ("/team analytics", "Show team-wide KPIs"),
...
Use `/accessibility` or `--screen-reader` to enable narration. High-contrast palettes and plain-text summaries are WCAG AA compliant.
    ...
    project = Table(title="Projects & Pipelines", show_edge=False, expand=True)
    project.add_column("Command")
    project.add_column("Description")
