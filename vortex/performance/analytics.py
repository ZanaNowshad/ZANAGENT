...
TEAM_DB = Path.home() / ".vortex" / "teams" / "team-analytics.sqlite"


class TeamAnalyticsStore:
    """Persist collaborative metrics across agents."""

    def __init__(self, database: Path | None = None) -> None:
        self._db_path = database or TEAM_DB
        self._db_path.parent.mkdir(parents=True, exist_ok=True)
        self._lock = asyncio.Lock()
        self._initialise()

    async def record_entry(self, team_id: str, entry: Dict[str, Any]) -> None:
        payload = json.dumps(entry, default=str)
        tokens = float(entry.get("tokens", 0.0))
        minutes = float(entry.get("minutes", 0.0))
        cost = float(entry.get("cost", 0.0))
        actor = entry.get("actor") or entry.get("node") or "unknown"
        event = entry.get("event") or entry.get("summary") or "entry"
        timestamp = float(entry.get("timestamp", time.time()))
        await self._execute(
            """
            INSERT INTO team_entries(team_id, actor, event, tokens, minutes, cost, timestamp, metadata)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (team_id, actor, event, tokens, minutes, cost, timestamp, payload),
        )

    async def snapshot(self, team_id: str) -> Dict[str, Any]:
        rows = await self._fetch(
            """
            SELECT actor, SUM(tokens), SUM(minutes), SUM(cost), COUNT(*)
            FROM team_entries WHERE team_id=? GROUP BY actor
            """,
            (team_id,),
        )
        totals = {"tokens": 0.0, "minutes": 0.0, "cost": 0.0, "entries": 0}
        contributors: List[Dict[str, Any]] = []
        for actor, tokens, minutes, cost, count in rows:
            contributors.append(
                {
                    "actor": actor,
                    "tokens": float(tokens or 0.0),
            ...
            totals["tokens"] += float(tokens or 0.0)
            totals["minutes"] += float(minutes or 0.0)
            totals["cost"] += float(cost or 0.0)
            totals["entries"] += int(count or 0)
        return {"team_id": team_id, "totals": totals, "contributors": contributors}

    async def insights(self, team_id: str) -> List[str]:
        summary = await self.snapshot(team_id)
        totals = summary["totals"]
        insights = [
            f"Team {team_id} logged {totals['entries']} events consuming {totals['tokens']:.0f} tokens.",
            f"Time investment totals {totals['minutes']:.1f} minutes at an estimated cost of ${totals['cost']:.2f}.",
        ]
        if summary["contributors"]:
            leader = max(summary["contributors"], key=lambda c: c["entries"])
            insights.append(
                f"Most active node: {leader['actor']} ({leader['entries']} events, {leader['tokens']:.0f} tokens)."
            )
        return insights
    ...
    async def aggregate_by_event(self, team_id: str) -> Dict[str, Dict[str, float]]:
        rows = await self._fetch(
            """
            SELECT event, SUM(tokens), SUM(minutes), SUM(cost)
            FROM team_entries WHERE team_id=? GROUP BY event
            """,
            (team_id,),
        )
        return {
            ...
            for event, tokens, minutes, cost in rows
        }
