...
The project dashboard panel (toggle `Ctrl-P`) visualises milestones, pipeline
summaries, and recent governance checks. It is updated automatically when slash
commands such as `/project status`, `/project plan`, `/pipeline run`, or `/govern`
complete. The hotkey highlights the panel while `/ledger` (via `Ctrl-L`) renders the
team ledger inside the main pane.

### Lifecycle Commands

| Command | Description |
| ------- | ----------- |
| `/project status [id]` | Summarise project milestones, releases, and ledger totals. |
| `/project plan [id] <sources...>` | Generate a roadmap from backlog files. |
| `/project release <version>` | Record a release milestone. |
| `/pipeline run <name>` | Execute the configured pipeline stages. |
| `/pipeline status <name>` | Display the most recent pipeline outcome. |
| `/pipeline logs <name>` | Print stage logs captured during execution. |
| `/pipeline dashboard <name>` | Render an aggregated pipeline history. |
| `/release tag <version>` | Tag a release and update the dashboard. |
| `/release rollback <version>` | Roll back the latest release entry. |
| `/govern [project]` | Run governance policy checks. |

## Team Collaboration Shortcuts

`Ctrl-N` opens the session creation flow, `Ctrl-B` primes the command bar for a
team broadcast, and `Ctrl-L` prints the shared ledger. The team panel lists nodes,
roles, and repository attachments while highlighting read-only participants.

Slash commands:

- `/broadcast <message>` sends an annotated message to the current team via the
  agent network.
- `/team analytics` summarises node-level KPIs using the team analytics store.
- `/attach <repo>` and `/handoff <repo> <task>` coordinate cross-repository work.

## Shortcut Summary

| Shortcut | Action |
| -------- | ------ |
| `Ctrl-P` | Focus project dashboard panel |
| `Ctrl-R` | Prefill `/pipeline run` in the command bar |
| `Ctrl-Shift-L` | Prefill `/pipeline dashboard` and focus the command bar |
| `Ctrl-L` | Render the ledger summary |
| `Ctrl-B` | Start a team broadcast message |
| `Ctrl-N` | Create a new collaboration session |
| `Ctrl-Shift-R` | Search command history |

Use the command palette (`:`) to fuzzy search the expanded command surface including
project, pipeline, and governance actions.
